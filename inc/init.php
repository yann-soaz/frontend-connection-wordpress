...
function ys_connection_script () {
  ...
  if ( !empty($post->post_content) && has_shortcode( $post->post_content, 'ys_login') ) {
    ...
    if (!empty($_GET['token']) && !empty($_GET['login'])) {
      wp_enqueue_script( 'ys-reset', plugins_url( '/assets/reset.js', $ys_connection_file ));
      wp_localize_script( 'ys-reset', 'ys_login_object', array( 
      ...
      wp_localize_script( 'ys-connection', 'ys_login_object', array( 
        'ys_ajax_url' => admin_url( 'admin-ajax.php' ),
        'ys_redirect_url' => (!empty($ys_options['redirect_url'])) ? $ys_options['connection_url'] : home_url()
      ));
    }  else {
