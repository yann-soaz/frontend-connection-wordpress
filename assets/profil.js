document.addEventListener('DOMContentLoaded', function () {
  ...
  let changePassBtn = document.getElementById('change-password');
  if (changePassBtn) {
    changePassBtn.addEventListener('click', function (e) {
      e.preventDefault();
      let newPassDiv = document.getElementById('new-password');
      if (newPassDiv) {
  ...
  let formUser = document.getElementById('front-end-profil');
  if (formUser) {
    formUser.addEventListener('submit', function (e) {
      e.preventDefault();
      formUser.classList.add('loading');
      let datasToSend = new FormData(this);
      datasToSend.append('action', 'ysprofil');
